...
<body faiscoMobi="true" id="g_body" class="g_locale2052 mobiCol3">
	...
	<script type="text/javascript">
		...
		function addClient(){
			...
			if(clientPassword==""){
				document.getElementById("notice").style.display="block";
				document.getElementById("notice").innerHTML="请填写用户密码!";
				return 0;
			}
			if(operatorId==""){
				document.getElementById("notice").style.display="block";
				document.getElementById("notice").innerHTML="请选择用户所属运营商!";
				return 0;
			}
		$.post("addClient.do", {
			...
			function(data) {
				if (data == 1) {
					...
					document.getElementById("notice").style.display = "block";
					document.getElementById("notice").innerHTML = "用户名称已存在!";
				} else {
					alert("终端用户添加失败!");
				}
			});
	...
	<div id="g_web" class="g_web">
		...
		<div class='loginHeader webHeaderBg'>
			<a
				href='searchClient.do?userId=<%=rb.getUserId()%>&page=1&roleName=<%=rb.getRoleName()%>'
				class='g_close icon-gClose'></a><span class='title pageTitle'>添加终端用户
		</div>
							...
							<div class='formMiddleContent formMiddleContent9 moduleContent'
								...
								<div id='signupPage'>
									...
									<div class='signUpContent'>
										<div id='profileArea'>
											<form id="addClientForm" name="addClientForm"
												action="addClient.do" method="post">
												<div class="notice" id="notice" style="display:none"></div>
												<div class='g_globalLine '>
													<input type='text' maxlength='20' name='clientName'
														id='clientName'
														class='mustItem itemEdit userAddItem g_input'
														placeholder='请输入终端用户名称(必填)'><span
												...
												<div class='g_globalLine '>
													<input type='text'
														class='mustItem itemEdit userAddItem g_input'
														id='clientPassword' name='clientPassword' maxlength='20'
														placeholder='请输入终端用户密码(必填)'"><span
												...
												<div class='g_globalLine '>
													<input type='text'
														class='mustItem itemEdit userAddItem g_input'
														id='clientPhone' name='clientPhone' maxlength='11'
														placeholder='请输入终端用户电话'><span
														class='icon-phoneIcon g_mainColor'></span><span
														class='mustIcon'>&nbsp;</span>
												</div>
												<div class='g_globalLine '>
													<input type='text' maxlength='20' id='clientEmail'
														name='clientEmail'
														class='mustItem itemEdit userAddItem g_input'
														placeholder='请输入终端用户邮箱'><span
												...
												<div class='g_globalLine'>
													<span class='icon-clientIcon g_mainColor'></span> <label
														...
														class="itemEdit g_select  mustItem" id="selectOperator"
														onChange="setType()">
														<option value="">请选择用户所属运营商</option>
														<c:forEach var="list" items="${operatorList}"
															varStatus="status">
