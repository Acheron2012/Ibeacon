				...
				if(confirm("确定要删除该终端用户吗?")){
					$.post("deleteClient.do",{clientId:clientId},
				 	function(data){
				 		if(data==1){
							alert("终端用户删除成功!");
							document.getElementById("newsLine"+clientId).style.display="none";
							document.getElementById("sepLine"+clientId).style.display="none";
						}else{
							alert("终端用户添加失败!");
						}
				 	});
	...
	<div id="g_web" class="g_web">
		...
		<div class='loginHeader webHeaderBg'>
			<a href='login.do?userName=-1&password=-1&roleName=-1'
				class='g_close icon-gClose'></a><span class='title pageTitle'>终端用户信息列表
					...
					<div class='formMiddle formMiddle400'>
						...
						<div class="searchInputBar">
							...
							<div class="popupSearchWrap popupSearchWrap0">
								...
								<form action="searchClientByCondition" id="searchClientByConditionForm"
									name="searchClientByConditionForm" method="post">
									<input id="searchInput" class="popupSearchInput"
										name="keyword" placeholder="请输入终端用户名称…"
										style="padding-right: 1.8rem;">
										<input type='hidden' id="type" name='type' value='clientName'>
								...
								<div class="icon-mSearchBtn" id="icon-searchSubmit"
									style="cursor:pointer" onClick="searchClientByCondition();"></div>
							</div>
						</div>
															...
															<tbody>
																<tr>
																	<td width="84%"><div align="left">
																			<a hidefocus="true"
																				href="viewClient.do?clientId=${list.id}&page=${page}"
																				target="_self">${list.name}</a>
																		</div></td>
																	<td width="8%"><div align="right">
																			<img src="images/edit.png" width="20" height="20"
																				style="cursor:pointer"
																				onclick="javascript:window.location.href='updateClientBefore.do?clientId=${list.id}'">
																		</div></td>
																	<td width="8%"><div align="right">
																			<img src="images/cancel.png" width="20" height="20"
																				style="cursor:pointer"
																				onclick="javascript:deleteClient(${list.id})">
																		</div></td>
																</tr>
															</tbody>
														</table>
									...
									<c:choose>
										...
										<c:otherwise>
											<div class="pagePrev icon-pagePrev icon-cnListP">
												<a hidefocus="true"
													href="searchClient.do?userId=<%=rb.getUserId()%>&page=${page-1}&roleName=<%=rb.getRoleName()%>"></a>
											</div>
										</c:otherwise>
									</c:choose>
		...
		<a href="addClientBefore.do"><div class="flex1" align="right">
