	...
	<script type="text/javascript">
		...
		var _htmlFontSize = (function() {
			...
			function deleteOperator(operatorId){
				if(confirm("确定要删除该运营商用户吗?")){
					$.post("deleteOperator.do",{operatorId:operatorId},
				 	function(data){
				 		if(data==1){
							alert("运营商用户删除成功!");
							document.getElementById("newsLine"+operatorId).style.display="none";
							document.getElementById("sepLine"+operatorId).style.display="none";
						}else{
							alert("运营商用户添加失败!");
						}
				 	});
	...
	<div id="g_web" class="g_web">
		...
		<div class='loginHeader webHeaderBg'>
			<a href='login.do?userName=-1&password=-1&roleName=-1'
				class='g_close icon-gClose'></a><span class='title pageTitle'>运营商信息列表
					...
					<div class='formMiddle formMiddle400'>
						...
						<div class="searchInputBar">
							...
							<div class="popupSearchWrap popupSearchWrap0">
								...
								<form action="searchOperatorByCondition" id="searchOperatorByConditionForm"
									name="searchOperatorByConditionForm" method="post">
									<input id="searchInput" class="popupSearchInput"
										name="keyword" placeholder="请输入运营商用户名称…"
										style="padding-right: 1.8rem;">
										<input type='hidden' id="type" name='type' value='operatorName'>
								...
								<div class="icon-mSearchBtn" id="icon-searchSubmit"
									style="cursor:pointer" onClick="searchOperatorByCondition();"></div>
							</div>
						</div>
							...
							<div class="formMiddleContent formMiddleContent307 moduleContent"
								...
								<div class="newsList " id="newsList307">
									<c:forEach var="list" items="${oblist}">
										<div id="newsLine${list.id}" class="newsLine line">
											<div id="lineBody${list.id}"
															...
															<tbody>
																<tr>
																	<td width="84%"><div align="left">
																			<a hidefocus="true"
																				href="viewOperator.do?operatorId=${list.id}&page=${page}"
																				target="_self">${list.name}</a>
																		</div></td>
																	<td width="8%"><div align="right">
																			<img src="images/edit.png" width="20" height="20"
																				style="cursor:pointer"
																				onclick="javascript:window.location.href='updateOperatorBefore.do?operatorId=${list.id}'">
																		</div></td>
																	<td width="8%"><div align="right">
																			<img src="images/cancel.png" width="20" height="20"
																				style="cursor:pointer"
																				onclick="javascript:deleteOperator(${list.id})">
																		</div></td>
																</tr>
															</tbody>
														</table>
									...
									<c:choose>
										...
										<c:otherwise>
											<div class="pagePrev icon-pagePrev icon-cnListP">
												<a hidefocus="true"
													href="searchOperator.do?userId=<%=rb.getUserId()%>&page=${page-1}&roleName=<%=rb.getRoleName()%>"></a>
											</div>
										</c:otherwise>
									</c:choose>
		...
		<a href="addOperatorBefore.do"><div class="flex1" align="right">
